...
class KeywordsNotPresentApiController extends Controller
{
    use HasAction;
    ...
    {
        $query = $this->repository->getHostKeywordsNotPresentQuery($project->host);
        ...
        $query = $this->pipeline->send($query)
            ->through([
                ...
                (new OptionFilter('competitors'))
                    ->multiple()
                    ->setColumn('compKW.host_id')
                    ->setMethod('whereIn'),
            ])
            ->thenReturn();

        return $this->processingAction($request, $query, 'compKW.keyword_id') ?:
